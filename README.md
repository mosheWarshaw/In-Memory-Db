...
```Col``` uses ```Column``` under the hood, so at the lowest level is the ```Column``` class abstracting the data type of the column and preventing boxing being done from all other parts of the code that need to be able to operate regardless of the data type.
<br>
The interfaces ```ICol``` and ```IColumn``` are used by these other parts because they need to be working on them regardless of the type.
...
<br>
<br>
```Column``` holds the cells as their primitive selves (excpet strings) without boxing (and wasting space) (in C#, generics can be primitives), and abstracts from all the other code what data type it is and the management of it in order for oprations to not be wasteful with booxing.
<br>
